...
class Login_model extends CI_Model
{
     ...
     * @param string $login : This is name of the user company
	...
	/**
     * This function is used to get customer information by email-id for forget password email
     * @param string $login : Login name acess on page login
     * @return object $result : Information of user sissaude
     */
    function getUserInformationByLoginName($login)
    {
        $this->db->select('*');
        $this->db->from('tbl_users, tbl_roles');
        $this->db->where('isDeleted', 0);
        $this->db->where('tbl_users.roleId', 'tbl_roles.roleId', FALSE);
    ...
    function getUserInformationByLoginNameAndPassword($login, $password)
    {
        $this->db->select('*');
        $this->db->from('tbl_users, tbl_roles');
        $this->db->where('isDeleted', 0);
    ...
    function getUserInformationByUserId($userId, $password)
    {
        $this->db->select('BaseTbl.userId, BaseTbl.password, BaseTbl.name, BaseTbl.roleId, Roles.role');
        $this->db->from('tbl_users as BaseTbl');
        $this->db->join('tbl_roles as Roles','Roles.roleId = BaseTbl.roleId');
